        ...
        message.guild.members.fetch(message.author.id).then(u => {
            ...
            if ((message.author.tag === data.creator)||(u.permissions.has('32'))) {
                ...
                if (wn === 0) {
                    ...
                    let ecemb = new MessageEmbed().setTitle(`Results for poll ${data.name}`)
                    .setDescription(`The poll ${winners.length > 1 ? 'winners are: ' : 'winner is'}\n\n **${winners.join(', ')}**\n\nSubmitted by ${data.creator}.`)
                    .setImage(`https://quickchart.io/chart?c={type:'pie',data:{labels:${JSON.stringify(data.options)},datasets:[{data:${JSON.stringify(preserved)}}]}}`)
                    .setFooter(`Poll ID: ${data.id} • Bot developed by Arcerion#7298 🖤`)
